...
correct_answers = 0
start_time = 0
incorrect_answers_count = 0  # Variable to count incorrect answers


def start_quiz_with_difficulty(difficulty):
    # Initialize variables
    global correct_answers, start_time, incorrect_answers_count
    correct_answers = 0
    incorrect_answers_count = 0
    start_time = time.time()

    # Filter questions based on difficulty
    selected_questions = list(questions.items()) if difficulty == "Hard" else random.sample(list(questions.items()), 4)
...
def ask_question(questions_list):
    if questions_list:
        ...
        create_question_window(question, choices)
    else:
        # No more questions, show quiz report
        show_quiz_report()


def create_question_window(question, choices):
    root = tk.Toplevel()
    root.title("Quiz")
    ...
    for choice in other_choices:
        choice_button = ttk.Button(root, text=choice, command=lambda btn_text=choice: check_answer(btn_text, choices))
        choice_button.pack(fill="x", padx=20, pady=5)  # Reduced pady for better spacing


def check_answer(choice, choices):
    global correct_answers, incorrect_answers_count
    user_answer = choice
    ...
    if user_answer == correct_choice:
        ...
        incorrect_answers_count += 1  # Increment the count of incorrect answers

    ask_question(choices[1:])  # Move to the next question by slicing the list
...
def show_quiz_report():
    ...
    total_questions_asked = correct_answers + incorrect_answers_count  # Calculate total questions asked
    ...
    report_text += f"Incorrect Answers: {incorrect_answers_count}\n"
    ...
    open_youtube_video()  # Open youtube video for incorrect answers
...
def open_youtube_video():
    ...
    for question in questions:
        if question not in related_answers or question not in questions:
            continue
        if question not in urls:
            continue
        url = urls[question]
