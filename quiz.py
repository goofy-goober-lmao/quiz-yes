...
def start_quiz_with_difficulty(difficulty):
    ...
    global correct_answers, report, start_time, incorrect_questions
    correct_answers = 0
    incorrect_questions = []
    report = ""
...
def create_question_window(question, choices, remaining_questions):
    root = tk.Toplevel()
    root.title("Quiz")
    ...
    for choice in other_choices:
        choice_button = ttk.Button(root, text=choice, command=lambda btn_text=choice: check_answer(btn_text, choices, remaining_questions))
        choice_button.pack(fill="x", padx=20, pady=5)  # Reduced pady for better spacing



def check_answer(choice, choices, remaining_questions):
    user_answer = choice
    for answer, explanation in choices:
        if answer == user_answer:
            if answer == choices[0][0]:  # Get the correct answer from the first item in the choices list
                global correct_answers
                correct_answers += 1
                break  # Exit the loop when the correct answer is found

    ask_question(remaining_questions)
